...
async function getApp() {
  if (!appPromise) { appPromise = import('./server.js').then(m => { const app = m.default || m; if (typeof app.use === 'function') { app.use(staticMiddleware); } return app; }); }
  return appPromise;
}
...
export default {
  async fetch(request) {
    ...
    try { const result = app(req, res); if (result && typeof result.then === 'function') await result; } catch (e) { return new Response('Internal Server Error: ' + e.message, { status: 500 }); }
